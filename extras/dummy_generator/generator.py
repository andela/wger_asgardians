...
user_parser.add_argument('--country',
                         action='store',
                         default='germany',
                         help=('What country the generated users should '
                               'belong to. Default: Germany'),
...
weight_parser.add_argument('--add-to-user',
                           action='store',
                           help='Add to the specified user-ID, not '
                           ' all existing users')
weight_parser.add_argument('--base-weight',
                           action='store',
                           help='Default weight for the entry generation,'
                           ' default = 80',
                           type=int,
                           default=80)
...
if hasattr(args, 'number_users'):
    ...
    with open(os.path.join('csv', 'first_names_{0}.csv'.format(args.country)))\
            as name_file:
        name_reader = csv.reader(name_file)
        for row in name_reader:
            first_names.append(row)

    with open(os.path.join('csv', 'last_names_{0}.csv'.format(args.country)))\
            as name_file:
        name_reader = csv.reader(name_file)
        for row in name_reader:
...
if hasattr(args, 'number_weight'):
    ...
    for user in userlist:
        ...
        existing_entries = [i.date for i in WeightEntry.objects.filter(
            user=user)]
