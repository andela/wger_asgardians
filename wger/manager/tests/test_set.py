...
class SetDeleteTestCase(WorkoutManagerTestCase):
    ...
    def delete_set(self, fail=True):
        """Define Helper function to test deleting a set from a workout."""
...
class TestSetOrderTestCase(WorkoutManagerTestCase):
    """Tests that the order of existing sets in a workout is preservead when adding new ones."""

    def add_set(self, exercises_id):
        """Define Helper function that adds a set to a day."""
        nr_sets = 4
        post_data = {'exercises': exercises_id,
    ...
    def get_order(self):
        """Define Helper function that reads the order of the the sets in a day."""
    ...
    def test_set_order(self, logged_in=False):
        """Define Helper function that add some sets and checks the order."""
        self.user_login('test')
        orig = self.get_order()
...
class TestSetAddFormset(WorkoutManagerTestCase):
    """Tests the functionality of the formset mini-view that is used in the add set page."""
