...
class ScheduleModelTestCase(WorkoutManagerTestCase):
    ...
    def test_get_workout_steps_test_4(self):
        ...
        Start is too far in the past, schedule ist not a loop
    ...
    def test_get_workout_steps_test_5(self):
        ...
        Start is too far in the past but schedule is a loop
...
class SchedulePdfExportTestCase(WorkoutManagerTestCase):
    ...
    def export_pdf_token(self, pdf_type="log"):
        """Define Helper function to test exporting a workout as a pdf using tokens."""
    ...
    def export_pdf(self, fail=False, pdf_type="log"):
        """Define Helper function to test exporting a workout as a pdf."""
        response = self.client.get(reverse('manager:schedule:pdf-{0}'.format(pdf_type),
                                           kwargs={'pk': 1}))
    ...
    def export_pdf_with_comments(self, fail=False, pdf_type="log"):
        """Define Helper function to test exporting a workout as a pdf, with exercise coments."""
        user = User.objects.get(username='test')
        uid, token = make_token(user)
        response = self.client.get(reverse('manager:schedule:pdf-{0}'.format(pdf_type),
                                           kwargs={'pk': 3,
    ...
    def export_pdf_with_images(self, fail=False, pdf_type="log"):
        """Define Helper function to test exporting a workout as a pdf, with exercise images."""
        user = User.objects.get(username='test')
        uid, token = make_token(user)
        response = self.client.get(reverse('manager:schedule:pdf-{0}'.format(pdf_type),
                                           kwargs={'pk': 3,
    ...
    def export_pdf_with_images_and_comments(self, fail=False, pdf_type="log"):
        """Define Helper function to test exporting a workout as a pdf, with images and comments."""
        user = User.objects.get(username='test')
        uid, token = make_token(user)
        response = self.client.get(reverse('manager:schedule:pdf-{0}'.format(pdf_type),
                                           kwargs={'pk': 3,
