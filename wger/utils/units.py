...
class AbstractWeight(object):
    ...
    Internally, all values are stored as kilograms and are converted
    only if needed. For consistency, all results are converted to python
    decimal and quantized to four places
    ...
    @staticmethod
    def normalize(value):
        """Help method that returns quantized.
