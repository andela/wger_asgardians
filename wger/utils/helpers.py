...
class DecimalJsonEncoder(json.JSONEncoder):
    ...
    This class is needed because we store some data as a decimal (e.g.
    the individual weight entries in the workout log) and they need to
    be processed, json.dumps() doesn't work on them
...
def disable_for_loaddata(signal_handler):
    """Define Decorator to prevent clashes when loading data with loaddata and post_connect signals.

    See also: http://stackoverflow.com/questions/3499791/how-do-i
    -prevent-fixtures-from-conflicting
...
def next_weekday(date, weekday):
    """Define Helper function to find the next weekday after a given date.
...
def make_uid(input):
    """Small wrapper to generate a UID, usually used in URLs to allow for anonymous access."""
    return urlsafe_base64_encode(force_bytes(input))


def make_token(user):
    """Define Convenience function that generates the UID and token for a user.
...
def check_token(uidb64, token):
    """Check that the user token is correct.
...
def password_generator(length=15):
    """Define A simple password generator.
...
def check_access(request_user, username=None):
    """Define Small helper function to check that the current.

    (possibly unauthenticated) user can access a URL that the owner user shared the link.
