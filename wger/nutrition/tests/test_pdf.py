...
class NutritionalPlanPdfExportTestCase(WorkoutManagerTestCase):
    ...
    def export_pdf_token(self):
        """Define Helper function to test exporting a nutritional plan.

        Exports as a pdf using a token as access (no fails)
        """
    ...
    def export_pdf(self, fail=False):
        """Define Helper function to test exporting a nutritional plan as a pdf."""
        # Get a plan
        response = self.client.get(reverse('nutrition:plan:export-pdf',
                                           kwargs={'id': 4}))
