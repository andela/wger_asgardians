...
class NutritionPlan(models.Model):
    ...
    def get_absolute_url(self):
        """Return the canonical URL to view this object."""
    ...
    def get_calories_approximation(self):
        """Calculate the deviation from the goal calories and the actual.
...
class Ingredient(AbstractLicenseModel, models.Model):
    ...
    def clean(self):
        """Do a very broad sanity check on the nutritional values according to the following rules:.
    ...
    def compare_with_database(self):
        """Compare the current ingredient with the version saved in the database.
    ...
    def send_email(self, request):
        """Send an email after being successfully added to the database.

        (for user submitted ingredients only)
        """
...
class WeightUnit(models.Model):
    """Is A more human usable weight unit (spoon, table, slice...)."""
...
class MealItem(models.Model):
    ...
    def get_nutritional_values(self, use_metric=True):
        """Sum the nutrional info for the ingredient in the MealItem.
