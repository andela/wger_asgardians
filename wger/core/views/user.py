...
def trainer_login(request, user_pk):
    """Allow a trainer to 'log in' as the selected user."""
    user = get_object_or_404(User, pk=user_pk)
...
def logout(request):
    ...
    For temporary users, delete them.
...
def registration(request):
    """Form to allow for registration of new users."""
...
@login_required
def api_key(request):
    """Allow the user to generate an API key for the REST API."""
    context = {}
...
@login_required
def allow_fitbit(request, code=None):
    """Allow a user to import data from their fitbit account."""
    FITBIT_CLIENT_ID = "228NXD"
    FITBIT_SECRET = "3bd6d1353c957ced38e12204b743f7b7"
    template_data = {}
